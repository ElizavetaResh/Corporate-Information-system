...
class TaskController {
    ...
    async getTask (req, res){
        const tasks = await db.query(`select task.id as id,
                                        em_name as employer, 
                                        t_name as name, f_name as faculty, 
                                        SUBSTRING(t_description, 1, 200) AS description, 
                                        t_price as price, 
                                        u_name as workerName, 
                                        u_surname as workerSurname, 
                                        ed_name as education,
                                        t_date as date
                                      from faculty, task, employer, worker, public.user, education
                                      where (task.faculty_idfaculty = f_id) 
                                        ...
                                        AND task.education_ideducation = education.ed_id`)
        res.json(tasks.rows)
    }
    async getEmployers(req, res){
      const empoyers = await db.query("SELECT em_id, em_name from employer")
      res.json(empoyers.rows)
    }
    async getFaculties(req, res){
      const empoyers = await db.query("SELECT f_id, f_name from faculty")
      res.json(empoyers.rows)
    }
    async getFilteredTask(req, res){
      const employer_ids = req.body.employer_ids;
      const faculty_ids = req.body.faculty_ids;
